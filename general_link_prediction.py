...
if model == 'transc':
    ...
    allscores_sub = {}
    allscores_ins = {}
    with open('/mnt/data/nefeli/reproduction/on2vec-master/yago/con2id.pkl', 'rb') as fil:
        on2vec_con2id= pickle.load(fil)
...
def link_prediction(valid=True):
        ...
        with open('candidates.pkl', 'wb') as fil:
            pickle.dump(Candidates, fil)

        '''
        need_allscores = {}
        for rel in ['subclass', 'instance']:
            for h in Candidates[rel]['head']:
                if rel == ['subclass']:
                    id2x = id2con
                else:
                    id2x = id2ins 

                for t in Candidates[rel]['tail']:
                    need_allscores[(id2x[h], rel, id2con[t])] = 1

        pdb.set_trace()
        with open('need_allscores.pkl', 'wb') as fil:
            pickle.dump(need_allscores, fil)
        '''
                            ...
                            if len(testees) > 0:
                                ...
                                if type_constrain:
                                    if target_type == 'concept':
                                        ...
                                        candidate_names = [id2con[cand] for cand in candidate_idxs]
                                        ...
                                        candidate_names = [id2ins[cand] for cand in candidate_idxs]
                                        target_size = len(candidate_idxs)
                                ...
                                if model == 'transc':
                                    if target == 'head':
                                        if rel == 'subclass':
                                            score = [allscores_sub[(cand, giv)] for cand in candidate_idxs]
                                        elif rel == 'instance':
                                            score = [allscores_ins[(cand, giv)] for cand in candidate_idxs]
                                    else:
                                        if rel == 'subclass':
                                    ...
                                    if target == 'head':
                                        if rel == 'subclass':
                                            score = [allscores_sub[(cand, giv)] for cand in candidate_names]
                                        elif rel == 'instance':
                                            score = [allscores_ins[(cand, giv)] for cand in candidate_names]
                                    else:
                                        if rel == 'subclass':
                                            score = [allscores_sub[(giv, cand)] for cand in candidate_names]
                                        elif rel == 'instance':
                                            score = [allscores_ins[(giv, cand)] for cand in candidate_names]    
                                elif model == 'ddmm':
                                    
                                    if target == 'head':
                                        score = [ allscores.get((cand, giv),1000) for cand in candidate_names]
                                    else:
                                        score = [ allscores.get((giv, cand),1000) for cand in candidate_names]

                                _ = [s for s in score if s != 1000]
                                if len(_) == 0:
                                    mean_score = 0
                                else:
                                    mean_score = sum(_) / len(_)
                                #pdb.set_trace()
                                score = [ s if s != 1000 else mean_score for s in score]
                                #pdb.set_trace()
...
#[820, 840, 860, 880, 900, 920, 940, 960, 980]#[900]#[1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900]
...
for epc in epcs:
    ...
    if model == 'transc':
        ...
        allscores_path = '/mnt/data/nefeli/reproduction/ddmm/allscores_{}.pkl'.format(epc)
        with open(allscores_path, 'rb') as fil:
            allscores = pickle.load(fil)
    elif model == 'on2vec':
        allscores_path = '/mnt/data/nefeli/reproduction/on2vec-master/yago/allscores_{}.pkl'.format(epc)
        with open(allscores_path, 'rb') as fil:
            allscores = pickle.load(fil)
        allscores_sub = allscores['sub']
        allscores_ins = allscores['ins']
